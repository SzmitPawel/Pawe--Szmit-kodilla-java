...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        // when
        long sumOfDays = board.getTaskLists().stream().
                filter(taskList -> taskList.getName().equals("In progress")).
                flatMap(taskList -> taskList.getTasks().stream()).
                map(Task::getCreated).
                map(days -> ChronoUnit.DAYS.between(days, LocalDate.now())).
                reduce(0L, (sum, current) -> sum += current);

        long numbersOfDays = board.getTaskLists().stream().
                filter(taskList -> taskList.getName().equals("In progress")).
                flatMap(taskList -> taskList.getTasks().stream()).
                map(Task::getCreated).
                count();

        double expectedAverage = sumOfDays / numbersOfDays;

        // then
        assertEquals(30, sumOfDays);
        assertEquals(3, numbersOfDays);
        assertEquals(10, expectedAverage);
    }
}